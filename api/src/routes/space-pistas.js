...
// ─── FIXED ROUTES FIRST (before parametric /:torneoId) ───
...
router.post('/combates', async (req, res) => {
  try {
    ...
    if (!torneo_id || !pista_id || !alumno1_id) {
      return res.status(400).json({ success: false, error: 'torneo_id, pista_id y alumno1_id son requeridos' });
    }
    if (alumno2_id && alumno1_id === alumno2_id) {
    ...
    const row = await queryOne(`
      ...
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `, [torneo_id, pista_id, modalidad_id || null, hora || null, alumno1_id, alumno2_id || null]);
...
router.put('/combates/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { estado, ganador_id, puntaje_alumno1, puntaje_alumno2, observaciones, hora, modalidad_id, alumno2_id } = req.body;
    ...
    if (alumno2_id !== undefined) { fields.push(`alumno2_id = $${idx++}`); values.push(alumno2_id); }
    ...
    values.push(id);
    const row = await queryOne(`
      UPDATE torneo_combates SET ${fields.join(', ')} WHERE id = $${idx} RETURNING *
    `, values);

    if (!row) return res.status(404).json({ success: false, error: 'Combate no encontrado' });
...
router.delete('/combates/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const row = await queryOne('DELETE FROM torneo_combates WHERE id = $1 RETURNING *', [id]);
    if (!row) return res.status(404).json({ success: false, error: 'Combate no encontrado' });
    res.json({ success: true, data: row });
  } catch (err) {
...
router.post('/:torneoId', async (req, res) => {
  try {
    ...
    const row = await queryOne(`
      INSERT INTO torneo_pistas (torneo_id, numero, nombre) VALUES ($1, $2, $3) RETURNING *
...
// DELETE /:pistaId — delete pista
router.delete('/:pistaId', async (req, res) => {
  try {
    const { pistaId } = req.params;
    const row = await queryOne('DELETE FROM torneo_pistas WHERE id = $1 RETURNING *', [pistaId]);
    if (!row) return res.status(404).json({ success: false, error: 'Pista no encontrada' });
    res.json({ success: true, data: row });
  } catch (err) {
